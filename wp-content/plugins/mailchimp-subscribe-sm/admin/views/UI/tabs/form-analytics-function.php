...
function  ulpb_loadAnaytics($postID){
...
function ulpb_RenderAnalytics($postID, $loadGraphs, $uniqID = 'default'){

    $defaultPageAnalytics = ulpb_loadAnaytics($postID);
        ...
        <div class="analytics-card" >
            ...
            <p> <?php
                if ($totalPopUpClosedCount > 0 && $defaultPageAnalytics['allImpressions'] > 0) {
                    echo round( $totalPopUpClosedCount/$defaultPageAnalytics['allImpressions'], 2) . "%"; 
                }else{
    ...
    var lastSevenDates_<?php echo $uniqID; ?> = [<?php for ($i=0; $i <7 ; $i++) { echo "'".$lastThirtyDatesForChart[$i]."',";} ?>];
    lastSevenDates_<?php echo $uniqID; ?>.reverse();
    var lastSevenData_<?php echo $uniqID; ?> = [<?php for ($i=0; $i <7 ; $i++) { echo "'".$numberOfConversions[$i]."',";} ?>];
    lastSevenData_<?php echo $uniqID; ?>.reverse();

    var lastSevenDataImpressions_<?php echo $uniqID; ?> = [<?php for ($i=0; $i <7 ; $i++) { 
        if ($lastThirtyDaysImpressions[$i] > 0) {
    ...
    var lastSevenDataConversionRate_<?php echo $uniqID; ?> = [ <?php for ($i=0; $i <7 ; $i++) {
